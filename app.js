(() => {
  ...
  function renderAgenda(mode){
    ...
    const html=dates.map(d=>{const ev=events.filter(s=>eventOnDate(s,d));return `<div class="agenda-day"><div class="agenda-date">${new Intl.DateTimeFormat('ko-KR',{month:'long',day:'numeric',weekday:'short'}).format(d)}</div>${ev.length?ev.map(s=>`<div class="agenda-item"><strong>${escapeHtml(s.complex)}</strong><span>${escapeHtml(s.category)}</span><span>${escapeHtml(s.memo||'-')}</span></div>`).join(''):'<div class="agenda-empty">등록된 일정이 없습니다.</div>'}</div>`}).join(''); $('#calendarContainer').innerHTML=`<div class="agenda">${html}</div>`;
  }
